...
# Setup mcp-accurate-click-server on Arch, Manjaro, Endeavour OS
################################################################################
...
install_system_packages() {
    ...
    local packages=(
        ...
        "xorg-xrandr"      # Monitor detection
        "xorg-xwininfo"    # X11 utilities
        "xorg-xprop"       # X11 properties tool
        "libx11"           # X11 libraries
        "python-pip"       # Python package manager (if needed)
    )
...
verify_installation() {
    ...
    for tool in xdotool wmctrl xrandr xwininfo xprop; do
...
post_installation_info() {
    ...
    echo ""
    echo "1. If using Manjaro or custom kernel:"
    echo "   - Ensure X11 is properly configured"
    echo "   - Check xrandr output: xrandr"
    echo ""
    ...
    echo "   - Check X11 display: echo \$DISPLAY"
    echo "   - Verify tools: which xdotool wmctrl"
    echo ""
...
main() {
    echo ""
    log_info "Arch Linux Setup Script for mcp-accurate-click-server"
    echo ""
