...
# Setup mcp-accurate-click-server on Fedora, RHEL, CentOS, AlmaLinux, Rocky
################################################################################
...
detect_package_manager() {
    ...
    if command -v dnf &> /dev/null; then
        PKG_MGR="dnf"
        log_success "Detected DNF (modern)"
    elif command -v yum &> /dev/null; then
        PKG_MGR="yum"
        log_warning "Detected YUM (legacy, consider upgrading to Fedora 22+)"
    else
...
update_packages() {
    ...
    if [ "$PKG_MGR" = "dnf" ]; then
        dnf check-update || true  # Don't fail if no updates
    else
        yum check-update || true  # Don't fail if no updates
    fi
...
install_system_packages() {
    ...
    # Different package names for different RHEL versions
    local x11_devel="libX11-devel"
    local xrandr_pkg="xrandr"

    local packages=(
        "xdotool"           # Input simulation
        "wmctrl"            # Window management
        "libxrandr"         # Monitor detection (fallback name)
        "$x11_devel"        # X11 development libraries
    ...
    if [[ "$DISTRO_NAME" == *"Fedora"* ]]; then
        packages+=("libxcb-devel")  # Additional X11 libraries
    fi
    ...
    if [ "$PKG_MGR" = "dnf" ]; then
        ...
        yum install -y "${packages[@]}" || {
            log_error "Failed to install system packages via yum"
            exit 1
        }
    fi
...
verify_installation() {
    ...
    for tool in xdotool wmctrl xrandr; do
    ...
    # Check X11 tools (might be under different names)
    for tool in xwininfo xprop; do
        if command -v "$tool" &> /dev/null; then
            log_success "  ✓ $tool found"
        else
            log_warning "  ✗ $tool not found (may be in xorg-x11-utils)"
        fi
    done
...
post_installation_info() {
    ...
    echo "   $PKG_MGR install xorg-x11-utils  # For xwininfo, xprop"
    echo ""
...
main() {
    echo ""
    log_info "Fedora/RHEL Setup Script for mcp-accurate-click-server"
    echo ""
